...
### 思ったことについて書いておく
ocaml の sig と structure みたいにする必要はなさそう。
また、第一級の module を扱うのもしないほうが良い。
この module の目的は、
- 文章を構造化して、参照を簡単にすること
- 議論の仮定を抽象化して、後から適用可能にすること

なので、インターフェースとして共通化することはこの機能を拡張して実装するべきではなさそう？
（例えば、3章全体で20ぐらい定義があったとして、"同じ名前と同じ型を持つ"文章と共通化を図るためだけに sig を入れるべきではない、と思った。）
すでに言語自体は higher なので、 module functor みたいなものも普通の def で実装できると思う。
例えば、 deciable euqality を持つ型クラスを受け取って MultiSet を List で実装するとかは書けそう。
Propositional でない  definitional な equality の制約がつけられないと書きにくい場面はありそうだけれど、それは surface 側から elaboration をうまくばいけるのでは。
結論としては、共通化のための仕組みは module にはいらない。

なので、 ocaml 系の module ではない、 namespace とか symbol table とかの name resoltion を参考にした方がいいと思った。
重要なのは、参照の path を書きやすく読みやすくすることと、仮定への適用をどう書くか？
それと、文章を書くという目的だとあまりいらないけれど、隠蔽とかは必要かも？
ただそれも、公開範囲やアクセスのパスの書き方と、と公開の仕方だけ考えればよいので、あまり凝った理論はいらないかも。
...
- あるいは、 `$($)` を使ってもいい： `$( 1 + $( 2 + 3 $) $)`
  - これはつまり、 "(" ~ ")" の中をそのまま exp と思うことにするとパースに失敗するということ。
...
- 導出木の展開を考えるうえで、同じ証明を作らないといけなくなるとつらい。
  ...
  - ただこの場合、順番を入れ替えただけで壊れるようになる？
    - ある goal が"上の部分で"解かれていた時にその goal を適用してしまうようにすると、
      "上の部分"がなくなったらそこで goal が発生するようになる。これはよくない。
    - なので、 goal の証明を 2 回以上やりたくない場合には、"明示的に"保持する仕組みが欲しい。
    - かつ、 weakning だけでできるならそれぐらいは自動で出してもらいたい。
    - これは、各式ごとには変わらないから大丈夫という考えもある。（ definition の間で共通で導出を持たなければ大丈夫という話。）

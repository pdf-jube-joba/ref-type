...
# modulus 演算
mod 演算の定義時には、ちゃんとそれが入るかどうかを議論する必要がある。
ふつうは商集合として定義するけれど、商集合の扱いは別にやるので、ここでは leq による有限集合とする。

```
def (_ "div" _): (n: nat) -> (m: nat) -> nat := $"if" n < m "then" 0 "else" 1 + (n - m) div m$ with {
  // general recursion の形なので、停止性について議論しないといけない。
}
def (_ "mod" _): (n: nat) -> (m: nat) -> nat := $"if" n < m "then" n "else" (n - m) div m$ with {
  // general recursion の形なので、停止性について議論しないといけない。
}

...
## 一般再帰側を CBPV にした
- 構文レベルからわけた（混ざると本当にめんどくさいはず。）
- Rf がうまくいっているかわからない。（というか、帰納型周りの話はめんどくさいのでぶん投げた。）

## Set と Program について
ai による指摘として、 Set 側に RunStep に対応する continue の injectivity がないのでうまく示せなかったりするらしい。

なら、結局 Set 側に一式を入れて prec をつける方がいいと思った。

Reflection を本当に構文間の写像として定義するとちょっとやりにくいはずだが、どうしよう。
メタなことをやるならやっぱり "プログラム型" 自体が Set の側にあるといいのだが、めっちゃ矛盾しやすそう？

というか、構文間の写像としての Rf と、プログラム型自体を持ってくる仕組みを分ければよさそう。

box(p): Box(P) で Program 側の項を包む、 force(box(p)) で実行して Set 側に持ってくることにした。
Rf 自体は box を持ってくるときに well-teminated かどうかを判定するための、項の reduction の話にする。

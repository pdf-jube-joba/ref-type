...
`src/kernel` の型検査・変換可能性判定・正規化・Program 評価を高速化する。kernel を直接呼ぶベンチマークで基準を取り、ノードの深いコピー削減、型推論のキャッシュキー改善、比較の重複走査削減、評価位置の保持の順に取り組む。

この文書はコード調査に基づく計画であり、各候補の寄与率と改善率は未計測。段階ごとに変更前後を比較し、効果と複雑さを確認して採否を決める。最初の実装単位は計測基盤とノードのコピー削減とする。

## 現状

| 箇所 | 現在の処理 | 改善候補 |
| --- | --- | --- |
| `Arena::data` / `store` | `Data` を clone する。`fields` は `Vec<Vec<Child>>` で、読み取り時にも深いコピーが発生する。interner と arena にもデータを複製して保持する | 不変ノードを共有し、変更時だけコピーする |
...
| `evaluate` / `reduce_once` | 一段簡約のたびに根から評価位置を探索し、祖先ノードを再構築する。評価位置の列挙にも一時 `Vec` を作る | 評価位置をスタックに保持する |
...
最優先の実装候補。まず `Rc<Data>` などで不変ノードを共有する形を試し、読み取り時に `Vec<Vec<Child>>` を複製しないようにする。

- arena の各 partition と interner で同じノード実体を共有する。interner の比較・ハッシュは構造に基づく現在の意味を維持し、ポインタ同一性だけの比較にはしない。
- 読み取り用の内部 API を導入し、`infer_inner`、比較、簡約規則の判定などを移行する。公開の family 別 handle と node API は可能な範囲で維持する。
- `map_children` は子に変更が生じるまでは元データを保持し、変更がある場合だけ編集用コピーを作る。変換結果が同じなら元 handle を返す。
- `store` は interner のヒット時に不要な深いコピーを行わない。共有化後も構造ハッシュのコストは残るため、コピー削減の効果と分けて調べる。
- 読み取り中の再帰処理から `store` が呼ばれても、`RefCell` の借用衝突が起きない構成にする。arena の borrow を再帰呼び出し全体に持ち越さない。
- `Rc` の参照カウント・間接参照の増加と、必要な auto trait への影響も確認する。共有方式は測定結果と API の要件に合わせて確定する。

完了条件は、同一構造の interning と元ノードの不変性を維持し、対象ケースで割り当て削減を確認できること。実行時間と保持メモリも比較し、主要ケースで再現性のある悪化があれば共有の粒度を見直す。
...
- binder 名を無視する規則、子の depth、family・level、`computational_fields` による証明注釈の扱いを維持する。

...
Rust の実装は kernel、raw、syntax、elab、sema、cli の六つの crate に分割した。
...
宣言と module parameter の検査境界で kernel への登録を行い、[raw arena](../src/raw/src/exp.rs) の作業ノードを回収する。
...
| 3 | 実際に選択された通常参照、論理 binder、record field の occurrence を記録する。proof HIR、syntax context、展開 origin、Resolver interface の独立化が残る。 |
...
宣言側の root は [retention.rs](../src/raw/src/environment/retention.rs)、macro 側の root は [macros.rs](../src/elab/src/macros.rs) の `retained_raw_roots` で列挙する。

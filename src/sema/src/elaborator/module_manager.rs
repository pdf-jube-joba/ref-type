...
impl ModuleManager {
    ...
    pub(crate) fn get_moditem_record(
        ...
        Some(ModItemRecord {
            type_name: Identifier::new(name.clone()),
            inductive: *inductive,
            associated_definitions: associated_definitions
                .iter()
                .map(|(name, definition)| (Identifier::new(name.clone()), *definition))
                .collect(),
        })
    }
    ...
    fn bind_namespace_from(
        ...
        for (child_name, arguments) in calls {
            ...
            if env.module_unavailable(child) {
                return Err("module requires an unavailable declaration".into());
            }
...
pub(crate) fn resolve_access(
    ...
    let item = loop {
        if env.unavailable(module, name) {
            return None;
        }
        ...
        match item {
            ItemAccessResult::ProgramInductive(item) => {
                ItemAccessResult::Inductive(ModItemInductive {
                    type_name: Identifier::new(reference.to_owned()),
    ...
    match item {
        ModuleItem::Definition { name, definition } => {
            ItemAccessResult::Definition(ModItemDefinition {
                def_name: Identifier::new(name.clone()),
        ...
        ModuleItem::ProgramInductive {
            ...
            record_fields: record_fields
                .as_ref()
                .map(|fields| fields.iter().cloned().map(Identifier::new).collect()),
            type_name: Identifier::new(name.clone()),
            ctor_names: constructor_names
                .iter()
                .map(|name| Identifier::new(name.clone()))
...
mod tests {
    ...
    fn unparameterized_aliases_reuse_source_declarations() {
        ...
        let binding = manager
            .bind_namespace(
                ...
                &mut Vec::new(),
                None,
                vec![(Identifier::new("Source".into()), vec![])],
            )
            .unwrap();
    ...
    fn module_instantiation_requires_all_well_typed_named_arguments() {
        ...
        let call = |name: &str, arguments| vec![(Identifier::new(name.into()), arguments)];
        assert!(
            manager
                .bind_namespace(
                    &mut env,
        ...
        assert!(
            manager
                .bind_namespace(
                    ...
                    call(
                        "Parameterized",
                        vec![(Identifier::new("wrong".into()), wrong_argument.into())],
                    ),
                )
                .is_err()
        );
        ...
        assert!(
            manager
                .bind_namespace(
                    ...
                    call(
                        "Parameterized",
                        vec![(Identifier::new("A".into()), argument.into())],
                    ),
                )
                .is_ok()
        );
    ...
    fn inductives_from_two_bindings_are_the_same_type() {
        ...
        manager
            .add_inductive(
                &mut env,
                Identifier::new("Token".into()),
                vec![Identifier::new("token".into())],
                spec,
            )
            .unwrap();
        manager.publish_current_module(&mut env).unwrap();
    ...
    fn nested_namespace_materializes_parent_dependencies() {
        ...
        manager
            .add_def(
                &mut env,
                Identifier::new("parent_value".into()),
                DefinedConstant::Pts {
                    ty: set,
                    body: parameter_exp,
                },
        ...
        manager
            .add_def(
                &mut env,
                Identifier::new("child_value".into()),
                DefinedConstant::Pts {
                    ty: set,
        ...
        let binding = manager
            .bind_namespace(
                ...
                vec![
                    (
                        Identifier::new("Parent".into()),
                        vec![(Identifier::new("A".into()), argument.into())],
                    ),
                    (Identifier::new("Child".into()), vec![]),
                ],
            )
            .unwrap();
    ...
    fn outer_substitution_specializes_parameterized_imports() {
        ...
        let dependency = manager
            .bind_namespace(
                ...
                None,
                vec![(
                    Identifier::new("Param".into()),
                    vec![(Identifier::new("A".into()), outer_argument.into())],
                )],
            )
            .unwrap();
        manager
            .add_import(&mut env, Identifier::new("P".into()), dependency)
            .unwrap();
        let ItemAccessResult::Definition(imported_value) = manager
            .get_item(
                &env,
                &LocalAccess::Named {
                    access: Identifier::new("P".into()),
                    child: Identifier::new("value".into()),
                },
            )
            .unwrap()
        else {

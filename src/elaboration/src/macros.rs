            ...
            SExp::AccessPath { access, parameters } => {
                ...
                match resolve_access(env, module, access) {
                    Ok((_, ItemAccessResult::Expression(exp))) => {
                        ...
                        *node = SExp::ResolvedExp(exp.index() as u32);
                    }
...
impl ModuleManager {
    ...
    pub fn register_macro(
        ...
        walk_sexp_mut(&mut template, &mut |node| match node {
            SExp::AccessPath { access, .. }
            | SExp::RecordTypeCtor { access, .. }
            | SExp::IndCase { path: access, .. }
            | SExp::IndElimPrim { path: access, .. }
            | SExp::ProgramCase { path: access, .. } => self.record_template_access(env, access),
            _ => {}
        });
        ...
        walk_sexp_mut(&mut definition.template, &mut |node| match node {
            ...
            SExp::ResolvedExp(exp) => {
                let renamed = remap_all_global_ids(
                    env.arena(),
                    crate::raw::exp::Exp::from_index(*exp),
                    &remapping.definition_ids,
                    &remapping.inductive_ids,
                    &remapping.program_inductive_ids,
                );
                *exp = exp_subst_map(env.arena(), renamed, &remapping.substitutions).index() as u32;
            }
            _ => {}
        });
        definition

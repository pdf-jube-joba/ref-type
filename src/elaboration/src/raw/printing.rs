...
pub fn format_module(env: &CrateEnv, module: crate::raw::ids::ModuleId) -> String {
    if env.namespace_binding_id(module).is_some() {
        let binding = env.binding(module);
        let source = format_module(env, binding.source);
        ...
        let arguments = binding
            ...
            .map(|(parameter, argument)| {
                ...
                let value = match argument {
                    crate::raw::environment::ModuleArgument::Pts(term) => format_exp(env, *term),
                    crate::raw::environment::ModuleArgument::ProgramType(ty) => {
                        format_value_type(env, *ty)
                    }

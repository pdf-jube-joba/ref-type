use crate::{ast::*, parse::term_parse::TermParser};
use logos::Logos;
...
fn lex_all<'a>(input: &'a str) -> Result<Vec<SpannedToken<'a>>, String> {
    ...
    match errors.into_iter().next() {
        Some(error) => Err(format!("{} at {}..{}", error.msg, error.start, error.end)),
        None => Ok(tokens),
    }
}

fn lex_recovering(input: &str) -> (Vec<SpannedToken<'_>>, Vec<ParseError>) {
...
mod term_parse;
pub use recovery::{
    ParsedSource, SyntaxDiagnostic, SyntaxToken, SyntaxTokenKind, parse_source_recovering,
};
...
trait TokenCursor<'a>: Sized {
    ...
    fn expect_ident(&mut self) -> Result<Identifier, ParseError> {
        match self.next() {
            Some(token) => match token.kind {
                Token::Ident(name) => Ok(Identifier::new(name.to_owned()).with_span(SourceSpan {
                    start: token.start,
                    end: token.end,
                })),
                other => Err(ParseError {
...
impl<'a> Parser<'a> {
    ...
    fn parse_import(&mut self) -> Result<ModuleItem, ParseError> {
        let local = self.bump_if_token(Token::Period);
        ...
        let mut calls = vec![];
        let package = if !local && rooted.is_none() {
            let name = self.expect_ident()?;
            if self.peek() != Some(&Token::KeyWord("\\as")) {
        ...
        let path = if let Some(package) = package {
            ModuleInstantiatePath::FromPackage { package, calls }
        } else if let Some(import_name) = imported_parent {
    ...
    fn parse_module(&mut self) -> Result<Module, ParseError> {
        ...
        let body = if self.bump_if_token(Token::Semicolon) {
            ...
            if let Err(error) = self.expect_token(Token::RBrace) {
                if !self.recovery {
                    return Err(error);
                }
    ...
    fn parse_module_items_with_spans(
        ...
        loop {
            ...
            let start_pos = self.pos;
            let start = self.tokens.get(self.pos).map_or(0, |token| token.start);
            let item = match self.try_parse_module_item() {
                Ok(Some(item)) => item,
                Ok(None) if !self.recovery => break,
                result => {
                    let error = match result {
                        Err(error) => error,
                        _ => ParseError {
                            msg: "expected a module item".into(),
                            start,
                            end: self.span_at(self.pos).end,
                        },
                    };
                    ...
                    let name = self
                        ...
                        .and_then(|token| match token.kind {
                            Token::Ident(name) => {
                                Some(Identifier::new(name.into()).with_span(SourceSpan {
                                    start: token.start,
                                    end: token.end,
                                }))
                            }
                    ...
                    let message = error.msg.clone();
                    self.errors.push(error);
                    self.recover_declaration(start_pos, false);
                    ModuleItem::Error { name, message }
                }
            };
    ...
    fn recover_declaration(&mut self, start: usize, root: bool) {
        // Scan from the declaration start: the failing production may already
        // have consumed the next declaration keyword or a closing delimiter.
        let mut braces = 0usize;
        self.pos = start + 1;
        while let Some(token) = self.tokens.get(self.pos) {
            if let Token::KeyWord(keyword) = token.kind
                && (keyword == "\\module" || !root && is_declaration_keyword(keyword))
            {
                return;
            }
            match token.kind {
                Token::LBrace => braces += 1,
                Token::RBrace if braces > 0 => braces -= 1,
                Token::RBrace if !root => return,
                Token::Semicolon if braces == 0 => {
                    self.pos += 1;
                    return;
                }
                _ => {}
...
mod tests {
    ...
    #[test]
    fn inductive_terminator_preserves_following_declarations() {
        let modules = str_parse_modules(
            r"\module M {
            \inductive Empty: \Set := ;
        ...
        for (item, expected) in items.iter().zip([0, 2, 2]) {
            let ModuleItem::Inductive { constructors, .. } = item else {
                panic!("inductive");
            };
            assert_eq!(constructors.len(), expected);
        }
        assert!(matches!(&items[3], ModuleItem::Definition { name, .. } if name.as_str() == "one"));
    }
}

...
impl GlobalEnvironment {
    ...
    fn fresh_localscope_bind(&mut self) {
        self.local_scope.binded_vars.clear();
    }
    ...
    fn module_add_rec(&mut self, module: &Module) -> Result<(), ErrorKind> {
        ...
        for RightBind { vars, ty } in parameters.iter() {
            ...
            for v in vars {
                ...
                parameters_elab.push((v, ty_substed.clone()));
            }
        }
        ...
        for decl in declarations {
            ...
            match decl {
                ...
                ModuleItem::Inductive {
                    ...
                    self.push_binded_var(type_name.clone());

                    // elaborate parameters and indices
                    // binding is memorized in local scope
                    let indices_elab = self.elab_telescope(indices)?;
                    let parameter_elab = self.elab_telescope(parameters)?;

                    // elaborate constructors
                    let mut ctor_names = vec![];
                    let mut ctor_type_elabs = vec![];

                    for (ctor_name, rightbind, ends) in constructors {
                        let ctor_name_var: Var = ctor_name.into();
                        ctor_names.push(ctor_name_var.clone());

                        let telescope = self.elab_telescope(rightbind)?;
                        ...
                        let ends_elab = self.elab_exp(ends)?;
    ...
    fn elab_telescope(&mut self, v: &Vec<RightBind>) -> Result<Vec<(Var, Exp)>, ErrorKind> {
        let mut result = vec![];

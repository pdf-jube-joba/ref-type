...
impl GlobalEnvironment {
    fn solve_module_arguments(
        ...
        for (child_name, arguments) in calls.iter_mut() {
            ...
            if self.crate_env.module_unavailable(child) {
    ...
    pub(super) fn module_add_rec(&mut self, module: &Module) -> Result<(), ElaborationError> {
        ...
        let predeclared = self.predeclared_modules
            && self
                ...
                .is_some();
        if let Some(cause) = self.skipped_at(&module.header_source, module.span) {
            self.crate_env
                .mark_module_unavailable(self.module_manager.current(), cause);
        ...
        if self
            .crate_env
            .module_unavailable(self.module_manager.current())
        {
            self.processed_modules.insert(self.module_manager.current());
            self.module_manager.moveto_parent(&self.crate_env);
            return Ok(());
        ...
        for (index, decl) in declarations.iter().enumerate() {
            self.crate_env.clear_blocked_reads();
            if let Some(cause) = self.skipped_at(
                &module.source,
                module
                    .declaration_spans
                    .get(index)
                ...
                if let Some(name) = name {
                    self.crate_env.mark_unavailable(
                        self.module_manager.current(),
                        name.0.clone(),
                        cause,
                    );
                ...
                ModuleItem::Definition {
                    ...
                    if let Err(error) = pts_result {
                        if program_errors.is_empty()
                            || !matches!(error, ElaborationError::Message(_))
                        {
                            return Err(error);
                        }

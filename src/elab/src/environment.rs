...
pub struct NamespaceBinding {
    ...
    pub remapping: DeclarationRemapping,
}

#[derive(Debug, Clone, Default)]
pub struct DeclarationRemapping {
...
pub struct CrateEnv {
    ...
    arena: Arena,
    pub inference_cache: std::cell::RefCell<InferenceCache>,
    pub contexts: RefCell<ContextInterner<Exp>>,
    ...
    pub whnf_cache: RefCell<FxHashMap<Exp, Exp>>,
    symbols: Vec<String>,
...
impl CrateEnv {
    pub fn mark_unavailable(&mut self, module: ModuleId, name: String, cause: u64) {
        self.unavailable.insert((module, name), cause);
    }

    pub fn mark_module_unavailable(&mut self, module: ModuleId, cause: u64) {
        self.unavailable_modules.insert(module, cause);
    }

    pub fn unavailable(&self, module: ModuleId, name: &str) -> bool {
        if self.unavailable.is_empty() && self.unavailable_modules.is_empty() {
            return false;
        }
        if let Some(binding) = self.namespace_bindings.get(&module)
            && binding.source != module
            && self.unavailable(binding.source, name)
        {
            return true;
        }
        if let Some(cause) = self.unavailable.get(&(module, name.to_owned())) {
            self.blocked_reads.borrow_mut().insert(*cause);
            true
        } else {
            self.module_unavailable(module)
        }
    }

    pub fn module_unavailable(&self, mut module: ModuleId) -> bool {
        if self.unavailable_modules.is_empty() {
            return false;
        }
        loop {
            if let Some(cause) = self.unavailable_modules.get(&module) {
                self.blocked_reads.borrow_mut().insert(*cause);
                return true;
            }
    ...
    pub fn clear_blocked_reads(&self) {
        self.blocked_reads.borrow_mut().clear();
    }
    pub fn blocked_reads(&self) -> Vec<u64> {
        let mut reads = self
            .blocked_reads
            .borrow()
            .iter()
    ...
    pub fn context_id(&self, context: &super::exp::ExpContext) -> ContextId {
        self.contexts
            .borrow_mut()
    ...
    pub fn add_module_in_scope(
        ...
        crate::derivation::CheckSession::new(self, &mut context)
    ...
    pub fn resolve_import(&self, mut module: ModuleId, name: &str) -> Option<ModuleId> {
        loop {
            if self.unavailable(module, name) {
                return None;
            }
    ...
    fn check_definition(
        ...
        use crate::{derivation::CheckSession, program_derivation::ProgramCheckSession};
    ...
    pub fn reserve_lazy_definition(
        &mut self,
        module: ModuleId,
        source: DefId,
        substitutions: Vec<(ModuleParamId, ModuleArgument)>,
    ...
    pub fn set_lazy_definition_remapping(&mut self, id: DefId, remapping: DeclarationRemapping) {
    ...
    pub fn set_lazy_inductive_remapping(
        &mut self,
        id: InductiveId,
        remapping: DeclarationRemapping,
    ) {
    ...
    pub fn reserve_lazy_program_inductive(
        &mut self,
        module: ModuleId,
        source: ProgramInductiveId,
        substitutions: Vec<(ModuleParamId, ModuleArgument)>,
    ...
    pub fn set_lazy_program_inductive_remapping(
        &mut self,
        id: ProgramInductiveId,
        remapping: DeclarationRemapping,
    ) {
...
impl CrateEnv {
    pub fn definition_context(&self, module: ModuleId) -> crate::exp::ExpContext {
        if let Some(context) = self.checking_contexts.get(&module) {
        ...
        ancestors
            ...
            .filter_map(|p| match p.kind {
                ModuleParameterKind::Pts { ty } => {
                    Some(crate::exp::ExpContextEntry { var: p.name, ty })
                }
                ModuleParameterKind::ProgramType => Some(crate::exp::ExpContextEntry {
                    var: p.name,
                    ty: self.arena.sort(crate::sort::Sort::Set(0)),
                }),
                ModuleParameterKind::ProgramValue { ty } => {
                    crate::reflection::reflect_value_type(self, ty)
                        .ok()
                        .map(|ty| crate::exp::ExpContextEntry { var: p.name, ty })
                }
            })
            .collect()
    }

    pub fn parameter_ids(&self) -> Vec<ModuleParamId> {
        self.modules
            .iter()
            .enumerate()
            .flat_map(|(m, module)| {
    ...
    pub fn definition_ids(&self) -> Vec<DefId> {
        self.modules
            .iter()
            .enumerate()
            .flat_map(|(m, module)| {
    ...
    pub fn inductive_ids(&self) -> Vec<InductiveId> {
        self.modules
            .iter()
            .enumerate()
            .flat_map(|(m, module)| {
    ...
    pub fn datatype_ids(&self) -> Vec<ProgramInductiveId> {
        self.modules
            .iter()
            .enumerate()
            .flat_map(|(m, module)| {

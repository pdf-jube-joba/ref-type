...
pub fn remap_all_global_ids(
    ...
    let remap_computation_type = |ty: &mut ComputationType| {
        *ty = crate::program_calculus::remap_computation_type_global_ids(
            arena,
            *ty,
            definitions,
            program_inductives,
    ...
    match &mut node {
        ...
        ExpNode::BoxProgram {
            ...
            remap_computation_type(program_ty);
            *program = crate::program_calculus::remap_computation_global_ids(
                arena,
                *program,
                definitions,
                program_inductives,
...
fn same_node_shape(arena: &Arena, left: &ExpNode, right: &ExpNode) -> bool {
    match (left, right) {
        ...
        (ExpNode::BoxType { program_ty: left }, ExpNode::BoxType { program_ty: right }) => {
            crate::program_calculus::computation_type_is_alpha_eq(arena, *left, *right)
        }
        (
            ExpNode::BoxProgram {
                program_ty: left_ty,
            ...
            crate::program_calculus::computation_type_is_alpha_eq(arena, *left_ty, *right_ty)
                && crate::program_calculus::computation_is_alpha_eq(
                    arena,
                    *left_program,
                    *right_program,
                )
    ...
    match arena.get(exp) {
        ...
        ExpNode::IndCase {
            ...
            match arena.get(head) {
                ExpNode::IndCtor {
                    ...
                    .get(idx)
                    .copied()
                    .map(|branch| crate::utils::assoc_apply(arena, branch, fields)),
        ...
        ExpNode::ForceBox { program_ty, boxed } => match arena.get(whnf(env, boxed)) {
            ExpNode::BoxProgram {
                program_ty: actual,
                program,
            } if crate::program_calculus::computation_type_is_alpha_eq(
                arena, actual, program_ty,
            ) && crate::program_calculus::reduce_computation_once(env, program).is_none() =>
            {
                crate::reflection::reflect_computation(env, program).ok()
            }
            _ => None,
        },
            ...
            match (
                ...
                (
                    ...
                    (
                        ...
                        crate::program::ComputationTypeNode::Return { value_ty },
...
pub fn type_head_normal(env: &CrateEnv, ty: Exp) -> Exp {
    whnf_with_erasure(env, ty, true)
}

pub fn expose_product(env: &CrateEnv, ty: Exp) -> Option<(SymbolId, Exp, Exp)> {
    let arena = env.arena();
...
pub fn base_carrier(env: &CrateEnv, ty: Exp) -> Exp {
    let arena = env.arena();
    let mut current = type_head_normal(env, ty);
    loop {
        match arena.get(current) {

...
use crate::exp::*;
use crate::ids::{InductiveId, SymbolId};
use crate::inductive::eliminator_type;
use crate::program::{ComputationTerm, ComputationType, ComputationTypeNode};
...
fn check(
    ...
    error!(target: "ref_type::typing",
        context = %crate::printing::format_ctx(session.env(), session.context()),
        term = %crate::printing::format_exp(session.env(), term),
        inferred = %crate::printing::format_exp(session.env(), inferred_ty),
    ...
    match arena.get(term) {
        ...
        ExpNode::Equal { left, right } => {
            ...
            let Some(carrier) = common_ambient_carrier(session.env(), left_ty, right_ty) else {
                error!(target: "ref_type::typing",
                    context = %crate::printing::format_ctx(session.env(), session.context()),
                    left = %crate::printing::format_exp(session.env(), left),
                    right = %crate::printing::format_exp(session.env(), right),
                    left_type = %crate::printing::format_exp(session.env(), left_ty),
                    right_type = %crate::printing::format_exp(session.env(), right_ty),
...
fn check_closed_program_type(
    ...
    let result = nested.check_computation_type(program_ty);
...
fn check_closed_well_terminated_program(
    ...
    let reflected = crate::reflection::reflect_computation(session.env(), program)
...
fn infer_inductive_elimination(
    ...
    for (index, case) in cases.iter().enumerate() {
        ...
        let case_ty = if recursive {
            ...
            crate::inductive::case_type(arena, &constructor_ty, return_type, constructor, this)
        };

...
pub(super) fn format_output(env: &CrateEnv, output: &Output) -> String {
    match output {
        ...
        Output::Exp(exp) => printing::format_exp(env, exp.clone()),
        Output::ValueType(ty) => printing::format_value_type(env, ty.clone()),
        Output::ComputationType(ty) => printing::format_computation_type(env, ty.clone()),
        Output::ComputationTerm(computation) => {
            printing::format_computation(env, computation.clone())
        }

...
`std` は論理・データ・集合・自然数・代数・算術、`real` は実数、`topology` は位相空間を提供する。
各 package の entry point は `src/root.ref` であり、`real` と `topology` は `std` に依存する。
`../tests/library` は利用例を兼ねたライブラリ全体の型検査である。
...
| 分野 | ファイル | 内容 |
| --- | --- | --- |
| 論理 | `std/src/Logic/Proposition.ref`、`std/src/Logic/Law.ref`、`std/src/Logic/Rel.ref`、`std/src/Logic/Classical.ref`、`std/src/Logic/Equality.ref` | 命題、関係、法則、古典論理、等式 |
| データ | `std/src/Data/Bool.ref`、`std/src/Data/Pair.ref`、`std/src/Data/Sum.ref`、`std/src/Data/FinSet.ref` | 基本データ、直積、直和、有限集合 |
| 自然数 | `std/src/Nat.ref` と `std/src/Nat/` 以下 | 自然数の演算、仕様、法則 |
| 集合 | `std/src/Set/Quotient.ref` | 同値類、商の台集合、演算の relational image |
| 代数 | `std/src/Algebra/Monoid.ref`、`std/src/Algebra/Algebra.ref` | Monoid、Group、Semiring、Ring、Field |
| 算術 | `std/src/Arithmetic/Int.ref`、`std/src/Arithmetic/Rat.ref`、`std/src/Arithmetic/IntAlgebra.ref` と各子モジュール | 整数、有理数、整数の代数構造 |
| 実数 | `real/src/AxiomaticReals.ref`、`real/src/DedekindReal.ref`、`real/src/CauchyReal.ref` と各子モジュール | 公理的実数、Dedekind 実数、Cauchy 実数 |
| 幾何 | `topology/src/Topology.ref` | 位相空間 |

入れ子モジュールは論理上のパスと同じ場所に置く。例えば
`std/src/Arithmetic/Rat/Fractions/Operations.ref` の `\module Quotient;` の本体は
`std/src/Arithmetic/Rat/Fractions/Operations/Quotient.ref` にある。子ファイルは親のスコープを引き継ぐので、
...
同じ元宣言に convertible な引数を渡す module import は同じ型 instance を共有する。
...
```sh
cargo run --quiet -p cli -- libs/std
cargo run --quiet -p cli -- libs/real
cargo run --quiet -p cli -- libs/topology
cargo run --quiet -p cli -- tests/library
